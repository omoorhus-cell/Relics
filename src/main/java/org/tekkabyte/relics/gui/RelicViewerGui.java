...
public class RelicViewerGui {
    ...
    public void openAdmin(Player player, int page) {
        open(player, page, RelicViewerHolder.Mode.ADMIN, true);
    }

    public void openPlayer(Player player, int page) {
        openPlayer(player, page, true);
    }

    public void openPlayer(Player player, int page, boolean showLoadoutReturnButton) {
        open(player, page, RelicViewerHolder.Mode.PLAYER, showLoadoutReturnButton);
    }

    private void open(Player player, int page, RelicViewerHolder.Mode mode, boolean showLoadoutReturnButton) {
        ...
        Inventory inv = Bukkit.createInventory(
                new RelicViewerHolder(page, mode, player.getUniqueId(), showLoadoutReturnButton),
                54,
                ItemBuilder.parseLegacyText(title)
        );
    ...
    public boolean shouldShowLoadoutReturnButton(Inventory inv) {
        InventoryHolder holder = inv.getHolder();
        if (holder instanceof RelicViewerHolder rvh) return rvh.shouldShowLoadoutReturnButton();
        return true;
    }
