...
public class ExprEventRelicSlot extends SimpleExpression<Number> {

    @Override
    protected Number[] get(Event e) {
        if (e instanceof RelicEquipEvent equipEvent) {
            return new Number[]{equipEvent.getSlot()};
        }
        if (e instanceof RelicUnequipEvent unequipEvent) {
            return new Number[]{unequipEvent.getSlot()};
        }
        return new Number[0];
    }
